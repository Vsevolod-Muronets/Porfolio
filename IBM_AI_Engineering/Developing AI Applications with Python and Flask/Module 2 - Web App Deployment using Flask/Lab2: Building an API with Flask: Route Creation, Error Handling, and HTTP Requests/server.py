...
def delete_by_uuid(uuid):
    for person in data:
        if person['id']==str(uuid):
            data.remove(person)
            return {'message': f'Person with provided ID {uuid} deleted'}, 200
    return {'message': 'Person not found'}, 404
...
def add_by_uuid():
    ...
    for person in data:
        if person['id']==str(new_person['id']):
            return {'message': f'Person already in data'}, 422
...
def int_serv_err(error):
    return {'message': 'Internal server error occured'}, 500
